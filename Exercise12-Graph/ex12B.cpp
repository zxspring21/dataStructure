...
class Print {
 public:
  Print(int begin, int end) { begin_ = begin; end_ = end; } 
  inline void operator()(const vector<int> &seq) const {
    // TODO
  }
...
void PrintAllSequences(const Graph<int> &g, int begin, int end) {
  TraversalByDFS(g, Print(begin, end));
}
...
int main() {
  ...
  PrintAllSequences(g, 2, 3);
  cout << endl;

  PrintAllSequences(g, 1, 4);
  cout << endl;
  system("pause");
  return 0;
}
